...
if [ $? -eq 0 ]; then
  echo ""
  echo "✅ Digest actualizado exitosamente con checksum: ${CALCULATED_CHECKSUM}"
  echo ""
  echo "📝 Ahora puedes ejecutar terraform init de nuevo"
else
  echo ""
  ...
  if [ $? -eq 0 ]; then
    echo "✅ Digest eliminado. Terraform lo recalculará automáticamente."
  else
    echo "❌ Error al eliminar Digest. Verifica permisos de AWS."
  fi
fi
